...
exports.getOriginalType = exports.getPromotedType = exports.getVisibleCellsForPiece = exports.checkPromotion = exports.isValidMove = exports.initializeVisibleBoard = exports.initialBoard = void 0;
...
const initializeVisibleBoard = (board, playerSide) => {
...
exports.initializeVisibleBoard = initializeVisibleBoard;
const isValidMove = (from, to, piece, board) => {
    ...
    const checkPath = (rowStep, colStep) => {
    ...
    switch (piece.type) {
        ...
        case "香":
            return (colDiff === 0 && rowDiff * direction > 0 && checkPath(direction, 0));
        case "桂":
        ...
        case "飛":
            if (isStraightMove) {
                return checkPath(rowDiff !== 0 ? (rowDiff > 0 ? 1 : -1) : 0, colDiff !== 0 ? (colDiff > 0 ? 1 : -1) : 0);
            }
            return piece.promoted && Math.abs(rowDiff) <= 1 && Math.abs(colDiff) <= 1;
        case "馬":
            if (isDiagonalMove) {
...
const checkPromotion = (from, to, piece) => {
    const [fromRow] = from;
    const [toRow] = to;
    if (piece.promoted) {
...
const getVisibleCellsForPiece = (row, col, piece, board) => {
    const visibleCells = [];
    const addVisibleCell = (r, c) => {
        if (r >= 0 && r < 9 && c >= 0 && c < 9) {
            visibleCells.push([r, c]);
        }
    };
    const direction = piece.player === "先手" ? -1 : 1;
    const addLineOfSight = (startRow, startCol, rowStep, colStep) => {
        let r = startRow + rowStep;
        let c = startCol + colStep;
        while (r >= 0 && r < 9 && c >= 0 && c < 9) {
            addVisibleCell(r, c);
            if (board[r][c] !== null)
                break; // 駒があれば、そこで視線が遮られる
            r += rowStep;
            c += colStep;
        }
    };
...
exports.getVisibleCellsForPiece = getVisibleCellsForPiece;
const addGoldMoves = (row, col, direction, addVisibleCell) => {
    addVisibleCell(row + direction, col - 1);
    addVisibleCell(row + direction, col);
    addVisibleCell(row + direction, col + 1);
...
const addDiagonalMoves = (row, col, addVisibleCell) => {
    for (let i = 1; i < 9; i++) {
...
const addStraightMoves = (row, col, board, addVisibleCell) => {
    // 上方向
...
const addKingMoves = (row, col, addVisibleCell) => {
...
exports.getPromotedType = getPromotedType;
    ...
    for (let row = 0; row < 9; row++) {
        for (let col = 0; col < 9; col++) {
            ...
            if (piece && piece.player === opponent) {
                if ((0, exports.isValidMove)([row, col], kingPosition, piece, board)) {
                    return true;
                }
            }
        }
